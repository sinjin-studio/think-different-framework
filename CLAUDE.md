...
- `--no-wait` - Disable auto-wait on rate limit (default: wait and poll in autonomous mode)
...
- `--multistep` - Enable multi-step lens calls for tool-enabled lenses (experimental, off by default). Gives Observer, Empath, Skeptic, Mortal, Logician a two-step explore-then-respond deliberation
- `--no-progressive` - Disable progressive compression (on by default, compresses conductor/lens views in later turns)
...
`lib/conductor_loop.sh` implements the agentic orchestration loop. The conductor (`lenses/conductor.sh`) operates in two phases per turn:
- **Phase 1 (pre-lens):** Picks which lens speaks next and what instruction to give it
- **Phase 2 (post-lens):** Reacts to what the lens actually said - decides whether to trigger a mechanism, review, or end the session
...
**User questions:** The conductor can set `next_lens: "user"` to pause the session and ask the operator a clarifying question (max 2 per session). Used when genuinely stuck between divergent paths, not for validation.

Guard rails: max 35 turns, minimum 15 turns before ending, minimum 7 distinct lenses before grounding, two-strike transcendence (requires two consecutive signals before triggering early grounding), rate limit awareness. Auto-wait on rate limit (polls with backoff until credits reset, max 4 hours) - enables unattended overnight runs. Disable with `--no-wait`.
...
Before each session starts, provocations pass through a fact-checking step (`verify_provocation()` in `context/ground.sh`). This only runs when the provocation contains quantitative/factual claims (detected via regex) and web search is available. It:
1. **Verifies** every statistic, percentage, or factual claim via web search
